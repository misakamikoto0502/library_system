...
def userwin(user_id):
    ...
    #編輯borrowable狀態
    def edit_borrow(bookid,temp2,tempd2):
        ...
        # global borrowable
        # borrowable = 0
        registed["history"][user_id] =temp2
        registed["deadline"][user_id] =tempd2
        ...
        global borrowable
        borrowable = 0
        new_df['borrowable'][bookid] -=1
        registed.to_csv("registed.csv",encoding="big5")
    ...
    def borrowbook(bookid,borrowable):
        if borrowable == 1:
            ...
            # registed["history"][user_id].append((bookname,time)) #0是寫死的
            # registed["borrow_num"][user_id] += 1 #0是寫死的
            # registed["deadline"][user_id].append((bookid,time)) #0是寫死的
            # edit_borrow(int(bookid))
            # messagebox.showinfo('借書成功', ('Thank you!','Your borrow time:',time))
            try:
                eval(registed["deadline"][user_id])
            except:

                print(type(registed["history"][user_id]))
                time = datetime.date.today()
                bookname = new_df["題名"][int(bookid)]
                print(bookname)
                temp = registed["history"][user_id]
                temp2 = temp+[bookname]
                print(registed["history"][user_id])
                registed["borrow_num"][user_id] += 1 
                tempd = registed["deadline"][user_id]
                tempd2 = tempd+[(int(bookid),time+datetime.timedelta(days=7))]
                edit_borrow(int(bookid),temp2,tempd2)
                messagebox.showinfo('借書成功', ('Thank you!','Your borrow time:',time))
            else:
                print(type(registed["history"][user_id]))
                time = datetime.date.today()
                bookname = new_df["題名"][int(bookid)]
                print(bookname)
                temp = eval(registed["history"][user_id])
                temp2 = temp+[bookname]
                print(registed["history"][user_id])
                registed["borrow_num"][user_id] += 1 
                tempd = eval(registed["deadline"][user_id]) 
                tempd2 = tempd+[(int(bookid),time+datetime.timedelta(days=7))]
                edit_borrow(int(bookid),temp2,tempd2)
                messagebox.showinfo('借書成功', ('Thank you!','Your borrow time:',time))
    ...
    def returnbook(bookid,borrowable):
        if borrowable == 0:
            time = datetime.date.today()
            registed["borrow_num"][user_id] -= 1 #0是寫死的
            x = registed["deadline"][user_id] #0是寫死的
            for i, a in enumerate(x):
                if bookid == a[0]:
                    if i + 1 <= len(x):
                        registed["deadline"][user_id] = x[:i] + x[i+1:] #0是寫死的
                    else:
                        registed["deadline"][user_id] = x[:i] #0是寫死的
    ...
    def view_profile():
        ...
        tk.Label(frame1,text=str(len(registed["history"][user_id]))).pack() #0是寫死的
        history_tree=ttk.Treeview(frame1,columns=("0","1"))
        history_tree.heading("0",text="書名")
        history_tree.heading("1",text="借閱日期")

        for i in range(len(registed["history"][user_id])): #0是寫死的
            bookname=registed["history"][user_id][i][0]
            deadline=bookname=registed["history"][user_id][i][1]
            # borrowdate=registed["history"][user_id][i][1] #第一個0是寫死的
            # history_tree.insert("",index="end",text=i+1,values=(bookname,borrowdate))
            history_tree.insert("",index="end",text=i+1,values=(bookname,deadline))
        history_tree.pack()


        tk.Label(frame2,text="待歸還數量").pack()
        tk.Label(frame2,text=str(registed["borrow_num"][user_id])).pack() #0是寫死的
        deadline_tree=ttk.Treeview(frame2,columns=("0","1"))
        deadline_tree.heading("0",text="書名")
        deadline_tree.heading("1",text="歸還期限")

        for i in range(len(registed["deadline"][user_id])): #0是寫死的
            bookname=new_df["題名"][int(registed["deadline"][user_id][i][0])] #第一個0是寫死的
            # deadline=registed["deadline"][user_id][i][1]+datetime.timedelta(days=7) #第一個0是寫死的
            deadline=registed["deadline"][user_id][i][1]
...
def lobby():
    ...
    def verpw(id,pw):
        ...
        if registed.loc[id]['pw']==pw:
            ...
            messagebox.showerror(title="error",message="請先註冊或帳號或密碼輸入錯誤2!")
    ...
    def verid(id,pw):
        ...
        try:
            ...
            messagebox.showerror(title="error",message="請先註冊或帳號或密碼輸入錯誤1!")
        else:
            verpw(id,pw)
    ...
    def regist():
        ...
        def chkregist():
            ...
            if(count>3):         
                ...
                print(registed)
                registed.to_csv("registed.csv",encoding="utf-8")
                regist.destroy()
